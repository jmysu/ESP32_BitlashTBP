 ...
 */
IRAM_ATTR void dataReceivedCB(uint8_t* address, uint8_t* data, uint8_t len, signed int rssi, bool broadcast)
{
    if (len <= 0)
        return;
    switch (*data) {
    case myEspNowMsgType::PING:
    case myEspNowMsgType::PONG:  
        memcpy(&_EspNowRx, data, len);
        {
        uint16_t uID =   address[4] << 8 | address[5];
        _EspNowRx.macAddr64 =  uID; // chop mac address into 16bit
        _EspNowRx.rssi = rssi;
        _EspNowRx.ums  = millis();
        }
        break;
    ...
    isEspNowRXed = true; //Raise the RXed flag
    if (_isRxInfo) { //Need print out debug info?
        // Serial.printf("\n[%lu]EspNowRX: Type:0x%0X", millis(), *data);
        Serial.printf("\n[%lu]\tEspNow:0x%0X Time:%lu", millis(), _EspNowRx.msgType, _EspNowRx.ums);
        Serial.printf(" Rx:%3u RSSI:%2ddBm", len, rssi);
        //Serial.printf(" From:[" MACSTR "] ", MAC2STR(address));
        Serial.printf(" From:[%02X%02X] ", address[4], address[5]);
        Serial.printf("%s", broadcast ? "Broadcast" : "Unicast");
    }

    blinkLED20(2); // Do a 2ms LED20 blink
}
...
void broadcastHello()
{
    ...
    if (0 == quickEspNow.send(ESPNOW_BROADCAST_ADDRESS, (uint8_t*)&myData, sizeof(espnow_struct))) {
        ...
        macLeader = _myNodesTime->getKey(_myNodesTime->size() - 1); // Leader is the last one in nodes list
        if (myData.macAddr64 == macLeader) { // Leader do 100ms blink by himself
            uint32_t iums = myData.ums % 1000;
            _EspNowLeaderUms = myData.ums; // Update Leader's time
            if (((_EspNowLeaderUms / 1000) % 6) == 0) {
                tickBlinkLED21.once_ms(1002 - iums, blinkLED21, (uint8_t)250); // Scheduling for next blink, with 2ms RTTI adjustment
                tickBlinkLED1.once_ms(1002 - iums, blinkLED1, (uint8_t)250); // Scheduling for next blink, with 2ms RTTI adjustment
            }
        } else {
        ...
        if (!_myNodesTime->has(myData.macAddr64)) { // New member
            _myNodesAlive->put(myData.macAddr64, 1); // Add 1 to nodesAlive
            _myNodesRSSI->put(myData.macAddr64, myData.rssi); // Add rssi to nodesRSSI
...
/**
 * @brief broadcastPing
 *        send PING to FF:FF:FF:FF:FF:FF
 *
 */
void broadcastPing()
{
    espnow_struct myData;
    myData.msgType = myEspNowMsgType::PING;
    myData.macAddr64 = myMac;
    myData.ums = millis();
    ...
    quickEspNow.onDataSent(dataSentCB); // set callback for reporting status
    if (0 == quickEspNow.send(ESPNOW_BROADCAST_ADDRESS, (uint8_t*)&myData, sizeof(espnow_struct))) {
        Serial.printf(">>>>>>>>>> PING broadcasted!\n");
    } else {
        Serial.printf(">>>>>>>>>> PING NOT sent\n");
    }
}
/**
 * @brief unicastPong()
 *
 *        Echo back PONG to sender
 *
 *        EspNow: EspNow data RXed
 */
void unicastPong()
{
    espnow_struct myData;
    myData.msgType = myEspNowMsgType::PONG;
    myData.macAddr64 = _EspNowRx.macAddr64;
    myData.ums = _EspNowRx.ums;
    ...
    if (0 == quickEspNow.send(mArray, (uint8_t*)&myData, sizeof(espnow_struct))) {
        //Serial.printf ("[%lu]>>>", millis());
        //Serial.printf("MAC:%0X Millis:%lu\n", myData.macAddr64, myData.ums);
        Serial.printf(">>> PONG sent!\n");
    } else {
        Serial.printf(">>> PONG NOT SENT!\n");
    }
}
/**
